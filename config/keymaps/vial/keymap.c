...
/** Layer defined here becomes the auto-sniping layer when active */
...
// Bit field ranges: 2 bits = 0-3, 4 bits = 0-15, 5 bits = 0-31, 6 bits = 0-63
...
// Trackball idle timeout before returning to previous layer
#ifndef CHARYBDIS_AUTO_POINTER_LAYER_TRIGGER_TIMEOUT_MS
#define CHARYBDIS_AUTO_POINTER_LAYER_TRIGGER_TIMEOUT_MS user_config.auto_time
#endif


// Auto pointer layer trigger threshold. Lower = more sensitive. Range 0-7
...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  ...
  // ├──────────────────────────────────────────────────────┤               ├────────────────────────────────────────────┤
      KC_LCTL  , KC_LGUI  , KC_LALT  , LSFT_T(KC_LNG2)  , KC_SPC   ,LT(1,KC_TAB),    LT(1,KC_BSPC), KC_ENT,                       KC_RSFT
  // ╰──────────────────────────────────────────────────────┤               ├──────────────────────────────────────────────────────╯
  //    ╰──────────────────╯
  ),
  [LAYER_LOWER] = LAYOUT(
  ...
  // ╭────────────────────────────────────────────────                 ╭──────────────────────────────────────────────────────╮
        KC_ESC,    KC_1,    KC_2,    KC_3,    KC_4,                         KC_6,    KC_7,    KC_8,    KC_9,    KC_0,
  // ├────────────────────────────────────────────────                 ├──────────────────────────────────────────────
        KC_TAB,    KC_Q,    DPI_MOD, S_D_MOD,    KC_R,                         S_D_MOD, DPI_MOD,    KC_I,    KC_O,    KC_P,
  // ├────────────────────────────────────────────────                 ├──────────────────────────────────────────────
       KC_LSFT,    KC_LGUI, KC_LALT, KC_LCTL, KC_LSFT,                         QK_BOOT, EE_CLR, SNIPING, DRGSCRL, KC_SCLN,
  // ├──────────────────────────────────────────────────────┤               ├────────────────────────────────────────────┤
       KC_LCTL,    DRGSCRL, SNIPING, EE_CLR, QK_BOOT,    KC_B,         KC_BTN1, KC_BTN2,                               KC_COMM
  // ╰──────────────────────────────────────────────────────┤               ├──────────────────────────────────────────────────────╯
  //    ╰────────────────
),
...
// clang-format on
// Auto mouse layer detection: if abs(x) or abs(y) exceeds threshold,
// automatically switch to LAYER_POINTER mouse layer
// Only active when auto mode is enabled
#ifdef POINTING_DEVICE_ENABLE

#ifdef CHARYBDIS_AUTO_POINTER_LAYER_TRIGGER_ENABLE
// Callback for sensor data, allowing user code to intercept and modify. Returns mouse report.
report_mouse_t pointing_device_task_user(report_mouse_t mouse_report) {
    if(user_config.is_auto_enabled){
// If horizontal or vertical movement exceeds CHARYBDIS_AUTO_POINTER_LAYER_TRIGGER_THRESHOLD
            if (abs(mouse_report.x) > CHARYBDIS_AUTO_POINTER_LAYER_TRIGGER_THRESHOLD || abs(mouse_report.y) > CHARYBDIS_AUTO_POINTER_LAYER_TRIGGER_THRESHOLD) {
                // If auto mouse layer timer is 0
                if (auto_pointer_layer_timer == 0) {
                    layer_on(LAYER_MOUSE);
                }
                // Update auto mouse layer timer
...
void matrix_scan_user(void) {
    if(user_config.is_auto_enabled){
         // If timer is active and elapsed time >= CHARYBDIS_AUTO_POINTER_LAYER_TRIGGER_TIMEOUT_MS
        if (auto_pointer_layer_timer != 0 && TIMER_DIFF_16(timer_read(), auto_pointer_layer_timer) >= CHARYBDIS_AUTO_POINTER_LAYER_TRIGGER_TIMEOUT_MS) {
            // Reset auto mouse layer timer
...
// Auto-enable sniping mode on layer change to LAYER_POINTER
...
bool process_record_user(uint16_t keycode, keyrecord_t* record) {
    switch (keycode) {
        ...
        case AUTO_THRESHOLD:
            if(record->event.pressed) { // key down
                user_config.threshold_value +=  1;
            } // Increase trackball trigger threshold
            return false; // prevent further processing of keycode
        case TInfo:
            if(record->event.pressed) { // key down
                user_config.is_oled_enabled ^=  1;
            } // Toggle trackball settings display
            return false; // prevent further processing of keycode
        case T_SAVE:
            if(record->event.pressed) { // key down
                eeconfig_update_user(user_config.raw1);
...
#define IDLE_SPEED 20 // Below this WPM, idle animation plays
#define TAP_FRAMES 2
#define TAP_SPEED 40 // Above this WPM, typing animation triggers
#define ANIM_FRAME_DURATION 200 // Duration per frame in ms
