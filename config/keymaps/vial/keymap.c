...
//   2: Symbol (記号・テンキー補助)
//   3: RGB    (RGB LEDコントロール・Keyball設定)
//   4: Number (数字・ファンクションキー)
//   5: Numpad (テンキー)
// ============================================================
...
// DPI精密モード制御
// ============================================================
uint8_t normal_cpi_val    = 14;  // 通常: 1400 DPI
uint8_t precision_cpi_val = 8;   // 精密:  800 DPI

enum my_keycodes {
    PRC_SW  = KC_F23,  // 押している間だけ精密モード
    PRC_TOG = KC_F24,  // 精密モードのトグル
};
...
// 【Layer 0: Base】
//   Home Row Mods:
//     A=GUI(Cmd)  S=Alt  D=Ctrl  F=Shift  (左手)
//     J=Shift  K=Ctrl  L=Alt              (右手)
//   親指(左):
//     1:LT(RGB,BSPC)  2:LT(SYM,ESC)  3:LT(MSE,LALT)
//     4:LSFT_T(LNG2)  5:LT(NUM,SPC)
//
// 【マウスレイヤー切替方法】
//   ・右下 TG(LAYER_MOUSE): 押すたびにON/OFFトグル
//   ・右小指 LT(LAYER_MOUSE, KC_COMM): ホールドで一時起動
//   ・左親指3 LT(LAYER_MOUSE, KC_LALT): ホールドで一時起動
// ============================================================
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [LAYER_BASE] = LAYOUT_universal(
        ...
        KC_LCTL, GUI_T(KC_A) , ALT_T(KC_S) , CTL_T(KC_D) , SFT_T(KC_F) , KC_G   ,                    KC_H   , SFT_T(KC_J) , CTL_T(KC_K) , ALT_T(KC_L) , LT(LAYER_MOUSE, KC_COMM) , KC_BSLS,
        KC_LSFT, KC_Z        , KC_X        , KC_C        , KC_V        , KC_B   ,                    KC_N   , KC_M        , KC_COMM     , KC_DOT      , KC_SLSH                   , TG(LAYER_MOUSE),
                 LT(LAYER_RGB, KC_BSPC), LT(LAYER_SYMBOL, KC_ESC), LT(LAYER_MOUSE, KC_LALT), LSFT_T(KC_LNG2), LT(LAYER_NUMBER, KC_SPC),    _______, _______, LT(LAYER_NUMPAD, KC_MINS), LT(LAYER_SYMBOL, KC_ENT), LT(LAYER_RGB, KC_LBRC)
    ),

    // =========================================================
    // Layer 1: Mouse
    // TG(LAYER_MOUSE) でON/OFF、ホールドキーで一時起動
    // =========================================================
    [LAYER_MOUSE] = LAYOUT_universal(
        ...
        _______, _______   , _______   , _______         , LCTL(LALT(KC_SPC)), KC_F14  ,    KC_VOLD, KC_VOLU    , KC_MS_BTN3, KC_MUTE    , _______, TG(LAYER_MOUSE),
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    switch (keycode) {
        ...
        // --------------------------------------------------
        // DPI調整 (Keyball標準処理をハイジャック)
        // --------------------------------------------------
        case CPI_I100:
#ifdef POINTING_DEVICE_ENABLE
            if (record->event.pressed) {
                if (is_precision_mode) {
                    if (precision_cpi_val < 120) precision_cpi_val += 1;
                    keyball_set_cpi(precision_cpi_val);
                } else {
        ...
        case CPI_D100:
            ...
            if (record->event.pressed) {
                if (is_precision_mode) {
                    if (precision_cpi_val > 1) precision_cpi_val -= 1;
                    keyball_set_cpi(precision_cpi_val);
                } else {
